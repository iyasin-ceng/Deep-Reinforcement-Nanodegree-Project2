...
def dqn(agent,
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        for t in range(max_timesteps_per_ep):
            ...
            # forwarding the action to the environment
            env_info = env.step(action)[brain_name]

            # Taking the next state
            next_state = env_info.vector_observations[0]

            # taking reward
            reward = env_info.rewards[0]

            # check weather the episode has finished
            done = env_info.local_done[0]
            agent.step(state, action, reward, next_state, done)

            # cumulative total rewards into score variable
            score += reward

            #Taking the next_state and set it to state
            state = next_state
